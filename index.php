...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['folder'])) {
    ...
    // Klasörü ve içeriğini silme fonksiyonu
    if (is_dir($folderPath)) {
        function deleteFolder($folder) {
            foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($folder, FilesystemIterator::SKIP_DOTS), RecursiveIteratorIterator::CHILD_FIRST) as $fileinfo) {
    ...
    // Sayfayı yenileyerek klasörün silinmesini sağlar
    header('Location: ' . $_SERVER['PHP_SELF']);
    exit;
}

// Klasörleri okuma ve boyutlarını hesaplama
$handle = opendir(".");
$projectContents = '';
$projectsListIgnore = ['.', '..']; // Gerekirse daha fazla klasör ismi ekleyebilirsiniz
...
while (($file = readdir($handle)) !== false) {
    if (is_dir($file) && !in_array($file, $projectsListIgnore)) {
        $folderSize = getFolderSize($file); // Klasör boyutunu hesapla
        $modificationTime = filemtime($file); // Ekleme/modifiye tarihini al
	  ...
	  <a href="' . $file . '">	
		<div class="file">
            ...
            <span>' . $file . '</span>
            <div class="details">
                <span><span class="boyut">Boyut:</span> ' . round($folderSize / 1024, 2) . ' KB | <span class="tarih">Tarih:</span> ' . date("Y-m-d H:i:s", $modificationTime) . '</span>
            </div>
			<div class="rth">
...
if (empty($projectContents))
	...
	if(strpos($projectContents,"http://localhost/") !== false) {
		$projectContents .= '<div class="file"> <div class="file-icon">📁</div><span><a target="_blank" href="#">HATA</a></span> </div></div> ';
